...
var isGraphQLFunctionIdentifier = e => A.isIdentifier(e) && X.has(e.escapedText);
  ...
  var _unrollElement = e => {
    ...
    var s = function resolveIdentifierToGraphQLCall(e, t, i) {
      ...
      while (A.isIdentifier(a) && a !== r) {
        ...
        while (A.isPropertyAccessExpression(a.parent)) {
          a = a.parent;
        }
        if (A.isVariableDeclaration(a.parent) && a.parent.initializer && A.isCallExpression(a.parent.initializer)) {
          a = a.parent.initializer;
        } else if (A.isPropertyAssignment(a.parent)) {
          a = a.parent.initializer;
        } else if (A.isBinaryExpression(a.parent)) {
          a = A.isPropertyAccessExpression(a.parent.right) ? a.parent.right.name : a.parent.right;
        }
      }
...
function getAllFragments(e, t, i) {
  ...
  A.forEachChild(h, (e => {
    if (A.isVariableStatement(e) && e.declarationList && e.declarationList.declarations[0] && "documents" === e.declarationList.declarations[0].name.getText()) {
...
var Y = "object" == typeof performance && performance && "function" == typeof performance.now ? performance : Date;
...
var crawlChainedExpressions = (e, t, i, r, a) => {
  if (A.isPropertyAccessExpression(e.expression) && se.has(e.expression.name.text)) {
    var n = "reduce" === e.expression.name.text;
    var s = e.arguments[0];
    var o = [];
    if (A.isCallExpression(e.parent.parent)) {
      var l = crawlChainedExpressions(e.parent.parent, t, i, r, a);
      if (l.length) {
        o.push(...l);
      }
    }
    if (s && A.isIdentifier(s)) {
      var u = a.languageService.getProgram().getTypeChecker();
      var h = u.getSymbolAtLocation(s)?.valueDeclaration;
      if (h && A.isFunctionDeclaration(h)) {
        s = h;
      } else if (h && A.isVariableDeclaration(h) && h.initializer) {
        s = h.initializer;
      }
    }
    if (s && (A.isFunctionDeclaration(s) || A.isFunctionExpression(s) || A.isArrowFunction(s))) {
  ...
  return s = o.flatMap((s => {
    ...
    var u, _loop2 = function() {
      if (!n && (A.isReturnStatement(o) || A.isArrowFunction(o))) {
        ...
        o = o.parent;
      } else if (A.isPropertyAccessExpression(o) && se.has(o.name.text) && A.isCallExpression(o.parent)) {
        var s = o.parent;
        var u = [];
        ...
        if (A.isVariableDeclaration(s.parent) && !h) {
...
function getPersistedCodeFixAtPosition(e, t, i) {
  ...
  var d = l;
  if (!(d && A.isCallExpression(d) && d.arguments[0] && A.isStringLiteralLike(d.arguments[0]))) {
    return;
  }
  var g = generateHashForDocument(i, d.arguments[0], u, d.arguments[1] && A.isArrayLiteralExpression(d.arguments[1]) ? d.arguments[1] : void 0);
  var m = o.arguments[0];
  if (!m) {
    return {
      span: {
...
var generateHashForDocument = (e, t, i, r) => {
  if (r) {
    ...
    return b("sha256").update(print(parse(m))).digest("hex");
  }
};
...
var ce = [ 52001, 52004, 52003, ne, le, ue, he, fe ];
  ...
  if (r) {
    var p = findAllPersistedCallExpressions(a, i).map((t => {
      ...
      if (g.startsWith("sha256:")) {
        var m = generateHashForDocument(i, d.arguments[0], u, d.arguments[1] && A.isArrayLiteralExpression(d.arguments[1]) ? d.arguments[1] : void 0);
        if (!m) {
          return null;
        }
        if (`sha256:${m}` !== g) {
...
var runDiagnostics = (t, {nodes: i, fragments: r}, a, n) => {
  ...
  var l = i.map((t => {
    ...
    var m = new Set([ ...oe, ...n.config.clientDirectives || [] ]);
    var S = getDiagnostics(l, g, void 0, void 0, v).filter((e => {
